...
import PyMca.QtBlissGraph as QtBlissGraph
...
class ElementGraph(QtBlissGraph):
    ...
    def pickPoint(self, apoint):
        energy = apoint.x()
        self.pointSelected(apoint)

    def pointSelected(apoint):
        # to be overloaded by container widget
        pass
...
class ElementAnalysisBrick(BlissWidget):
    def __init__(self, *args):
        ...
        self.graph.pointSelected = self.pointSelected
    ...
    def pointSelected(self,point):
        self.selectedEnergy = point.x()
        print "<ElementAnalysiss> Selected energy is: ",self.selectedEnergy
    ...
    def moveCLFluor(self):
        logging.info("The current status is %s", self.hardwareObject.clfluorStateChannel.getValue())
        logging.info("The next status is %s", (not self.hardwareObject.clfluorStateChannel.getValue()))
...
#
    ...
    def drawNewGraph(self,xdata,ydata):
            ...
            #self.graph.setAxisOptions(qwt.QwtPlot.yLeft,qwt.QwtAutoScale.None)
            #
    ...
    def setYLogScale(self,state):
        if state:
#            self.graph.setAxisOptions(qwt.QwtPlot.yLeft,qwt.QwtAutoScale.Logarithmic)
